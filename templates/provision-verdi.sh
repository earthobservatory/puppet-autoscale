...
# read CustomData and do Base64 decode to get BUNDLE_URL
export BUNDLE_URL="$(sudo cat /var/lib/waagent/CustomData | base64 --decode | grep BUNDLE_URL | cut -d= -f 2 | sed -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//')"
export BUNDLE_CONTAINER="$(echo $BUNDLE_URL | cut -d/ -f 4)"
...
az storage blob download --container-name ${BUNDLE_CONTAINER} --file /tmp/${BUNDLE_BASENAME} --name ${BUNDLE_BASENAME}
